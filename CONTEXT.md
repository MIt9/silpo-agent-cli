...
The step, run before product search, that establishes which address/branch context the Reorder flow searches and prices against. Source is `silpo_get_my_delivery_addresses` (never order data, whose schema isn't confirmed to carry an address). The first/default saved address (as marked by MCP itself) is always proposed for confirmation first. If declined, the user picks from the remaining saved addresses or enters a new one via `silpo_find_address`. The confirmed choice is written to the Reorder log for audit, but doesn't change what's proposed next run — MCP's own default still wins. This precedes typical-item search because branch/delivery context determines product availability and pricing.
